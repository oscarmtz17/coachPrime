...
{
    // Obtener imágenes públicas de la categoría seleccionada
    var publicImages = await _s3Service.ListImagesByCategoryAsync(category);

    // Obtener imágenes privadas del usuario en la categoría seleccionada
    var userImages = await _s3Service.ListUserImagesByCategoryAsync(userId, category);

    // Combina las imágenes públicas y privadas
